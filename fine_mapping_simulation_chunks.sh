...
sample_size=2000000
model=bolt
model_name=fine_map_bolt_cheat
############################
...
#### Setting up files ####
experiment_name=small_chunks/chucks_1_marker_her_${he}_ind${number_of_indiduals}_casual_${casual_markers}
...
bayesr_output_path=$experiment_name/${model_name}
figure_output_class=$experiment_name/figures
casual_snp_info=$experiment_name/casual_snp_info
mkdir $experiment_name
mkdir $experiment_name/genome_regions_map
mkdir $experiment_name/genome_regions
for path in $genome_sampled_markers_path $genome_sample_binary_path $phenotype_path $gwas_sum_stats_path $bayesr_output_path $figure_output_class $casual_snp_info; do
    mkdir $path
done
###########################
...
ldak --calc-cors $bayesr_output_path/cor_sample_${i} --bfile ${genome_sample_binary_path}_${i} --keep rand.5000
ldak --mega-prs $bayesr_output_path/bayesr_sample_${i} --model bayesr --summary $gwas_sum_stats_path/gwas_sample_${i}.summaries --cors $bayesr_output_path/cor_sample_${i} --power 0.25 --high-LD highld.txt --allow-ambiguous YES --max-threads 4
#######################


#### Figures for each simulation ####
python -c "import python_scripts.utils as utils; utils.make_plots_for_genome_region('$gwas_sum_stats_path/gwas_sample_${i}.pvalues', '$bayesr_output_path/{$model_name}_sample_${i}.probs', '$figure_output_class/sample_${i}', '$casual_snp_info/snp_info_sample_${i}')"
done
#####################################
