...
parser.add_argument("-s", "--sample-size", help = "the number of base pairs to sample per genome region")
parser.add_argument("-n", "--sample-number", help = "the number of genome regions to sample")
...
sample_size, sample_number, genome_path, output_path = int(args.sample_size), int(args.sample_number), args.bfile, args.output_path
...
for i in tqdm(range(sample_number)):
    bounds.append(sample_snp(bounds, size=sample_size))
    with open(f'{output_path}_{i}', 'w') as f:
        f.write(f'{bounds[i][CHROMOSOME]} {bounds[i][START]} {bounds[i][FINISH]} sample_{i+1}')
