...
# makes genotypes and phenotypes
...
""" > scripts/job.1.make_genome_combine
...
""" > scripts/job.2.simulate_phenotypes.${he}_${casual_markers}
done; done


# makes snp ld matrix for sussie and finemap
...
""" > scripts/job.2.make_ld_matrix.${he}_${casual_markers}
...
""" > scripts/job.2.make_cors_ldak

# make cors for ldak combined
echo """$(cat base_slurm.sh)
$ldak --calc-cors $combined_samples/cors --bfile $combined_samples/combined_samples --max-threads 4 --keep $output_path/rand.5000

""" > scripts/job.2.make_cors_ldak_combined
...
""" > scripts/job.3.gwas_combined.${he}_${casual_markers}
done;done

# makes directories for models, and other files each model needs
echo """$(cat base_slurm_small.sh)
source simulate_chunks_const.sh
for he in \$HE_CHUNKED; do
for casual_markers in \$CASUAL_VALUES; do
source simulate_chunks_loop_const.sh
mkdir \$models_path
...
""" > scripts/job.5.test_models.bayesr_default.${he}.${casual_markers}
done; done
...
""" > scripts/job.5.test_models.bolt_anti_cheat.${he}.${casual_markers}
done;done
...
for ((i=0; i<$num_simulations; i++)); do
    ...
    if [[ \"$HE_COMBINE\" =~ $he ]]; then
        num_lines=\$(wc -l < $gwas_chunked_combine_pheno/sample_\${i}.summaries)
        casual_prop=\$(echo \"scale=7; $casual_markers/\$num_lines\" | bc)
        echo \"$he 0\$casual_prop 0\" > $chunked_compheno_models/bolt_cheat/params_\$i
        $ldak --mega-prs $chunked_compheno_models/bolt_cheat/sample_\${i} --model bolt --summary $gwas_chunked_combine_pheno/sample_\${i}.summaries --cors $cors_path/sample_\${i} --power 0.25 --high-LD highld.txt --allow-ambiguous YES --max-threads 4 --parameters $chunked_compheno_models/bolt_cheat/params_\$i
    fi

done
""" > scripts/job.5.test_models.bolt_cheat.${he}.${casual_markers}
done;done
...
""" > scripts/job.5.test_models.$model_name.${he}.${casual_markers}
done; done
...
rm $models_path/bolt_cheat_casual/params_\$i
touch $models_path/bolt_cheat_casual/params_\$i
for ((i=0; i<$num_simulations; i++)); do
...
""" > scripts/job.5.test_models.bolt_cheat_casual.${he}.${casual_markers}
done; done
...
for ((i=0; i<$num_simulations; i++)); do
    ...
    param_path=$models_path/bolt_no_cheat/params
    gwas=$gwas_sum_stats_path
    rm \${param_path}_\$i
    ...
    for he_t in 0.0005 0.001 0.002 0.004 0.008 0.016 0.032 0.064 0.128 0.256; do
        ...
        $ldak --mega-prs $models_path/bolt_no_cheat/sample_\${i} --model bolt --summary $gwas_sum_stats_path/sample_\${i}.summaries --cors $cors_path/sample_\${i} --power 0.25 --high-LD highld.txt --allow-ambiguous YES --max-threads 4 --parameters \${param_path}_\$i  
    fi

    if [[ \"$HE_COMBINE\" =~ $he ]]; then
        mkdir $chunked_compheno_models/bolt_no_cheat
        param_path=$chunked_compheno_models/bolt_no_cheat/params
        gwas=$gwas_chunked_combine_pheno
        rm \${param_path}_\$i
        touch \${param_path}_\$i
        ...
        $ldak --mega-prs $chunked_compheno_models/bolt_no_cheat/sample_\${i} --model bolt --summary $gwas_chunked_combine_pheno/sample_\${i}.summaries --cors $cors_path/sample_\${i} --power 0.25 --high-LD highld.txt --allow-ambiguous YES --max-threads 4 --parameters \${param_path}_\$i   
    fi

done
""" > scripts/job.5.test_models.bolt_no_cheat.${he}.${casual_markers}
done; done
...
mkdir $models_path/$model_name
rm $models_path/$model_name/params_\$i
touch rm $models_path/$model_name/params_\$i
for ((i=0; i<$num_simulations; i++)); do
    for he_t in 0.0005 0.001 0.002 0.004 0.008 0.016 0.032 0.064 0.128 0.256; do
    for ((as_c=1; as_c<=10; as_c++ )); do
        # make the params file for bolt
...
""" > scripts/job.5.test_models.susieR.${he}.${casual_markers}
done;done


# commented out becouse this configuration takes ages to run
...
#### genome wide models. #####
# the following model are fit to every chunk
# simulatanously.
...
""" > scripts/job.5.test_models.${model_name}_combine.${he}.${casual_markers}
done; done
...
""" > scripts/job.6.sum_stats.chunked.${he}.${casual_markers}
done; done
...
mkdir $experiment_name_chunk_compheno/stacked_figures
for ((i=0; i<$num_simulations; i++)); do
python3 -c \"import python_scripts.utils as utils; utils.make_stacked_plots('$gwas_chunked_combine_pheno', '$chunked_compheno_models', '$experiment_name_chunk_compheno/stacked_figures', '$casual_snp_info', \$i)\"
done
...
""" > scripts/job.6.sum_stats.comb.${he}.${casual_markers}
done; done
...
""" > scripts/job.6.fdr.${he}.${casual_markers}
done;done
